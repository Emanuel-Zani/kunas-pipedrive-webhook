...
async function buscarPersonaPorNombre(nombreBuscado) {
  try {
    console.log(
      `🔍 Buscando persona con nombre: "${nombreBuscado}" en Pipedrive...`
    );
    const response = await fetch(
      `${BASE_URL}/persons/search?term=${encodeURIComponent(
        nombreBuscado
    ...
    if (data.data?.items?.length > 0) {
      for (const itemObj of data.data.items) {
        ...
        if (persona.name.toLowerCase() === nombreBuscado.toLowerCase()) {
          const email =
            persona.emails?.find((e) => e.primary)?.value || "No especificado";
          console.log(
            `✅ Persona encontrada: ID ${persona.id}, Nombre: ${persona.name}, Email: ${email}`
          );
          return persona.id;
        }
      }
...
async function crearPersonaEnPipedrive(nombreCompleto, email) {
  ...
  try {
    const response = await fetch(
      `${BASE_URL}/persons?api_token=${PIPEDRIVE_API_KEY}`,
      {
        method: "POST",
...
export async function POST(request) {
  try {
    ...
    if (reservation.data_type === "avail") {
      ...
      return NextResponse.json(
        { message: "Evento 'avail' ignorado." },
        { status: 200 }
      );
    }
    if (
      reservation.data_type === "reservation" &&
      reservation.action === "insert"
    ) {
      const reservationId = reservation.data.id_reservations;
      if (processedReservations.has(reservationId)) {
        console.log(
          `✅ Reserva con ID ${reservationId} ya procesada. Ignorando.`
        );
        return NextResponse.json(
          { message: "Reserva ya procesada." },
          { status: 200 }
        );
      }
      ...
      const niños =
        (reservation.data.children_1 ?? 0) +
        (reservation.data.children_2 ?? 0) +
        (reservation.data.children_3 ?? 0) +
        (reservation.data.children_4 ?? 0) +
        (reservation.data.children_5 ?? 0) +
        (reservation.data.children_6 ?? 0) +
    ...
    return NextResponse.json(
      { message: "No se procesó la reserva." },
      { status: 200 }
    );
  } catch (error) {
...
async function addDeal(reservationDetails, personaId, niños) {
  const dealData = {
    ...
    "07536df1ea27b358d11a5bc03244c8ada11a2f91":
      reservationDetails.total_price.toString(),
    //Fecha de llegada
    ddc01baa72203eab75797469a79a1afc776dac68:
      reservationDetails.date_arrival ?? "No especificado",
    //Fecha de salida
    cd15dbfc3572ccb243664390a21010e3eb9e7c81:
      reservationDetails.date_departure ?? "No especificado",
    //Cantidad de noches
    ec929dafad8161a2191a9310e8a22c3f0e14dcea:
      reservationDetails.nights ?? "No especificado",
    //Cantidad de adultos
    aee0b941b3164ed351e8f73989bca903207a97f3:
      reservationDetails.adults ?? "No especificado",
    //Cantidad de niños
    "5f41eab7a51a40acbf99a24d8dc36a7f5786cf86": niños ?? "No especificado",
    //Teléfono
    phone: reservationDetails.phone ?? "No especificado",
    ac907fd34e67f90bab739453da5642cfc79dbf3a:
      reservationDetails.phone ?? "No especificado",
    //País
    "6c93fbe9ffc994b56c640ebf803cf63a0a0d67a4":
      reservationDetails.country ?? "No especificado",
    "91643604b4916086cf51d676af68bcb53b7c4d44":
      reservationDetails.country ?? "No especificado",
  ...
  try {
    const response = await fetch(
      `${BASE_URL}/deals?api_token=${PIPEDRIVE_API_KEY}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
